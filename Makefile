...
setup:            ## 의존성 설치 (backend: uv 우선, 없으면 venv+pip / frontend: npm)
	cd backend && (uv sync || (python3 -m venv .venv && .venv/bin/pip install -e .))
	cd frontend && npm install

data:             ## raw 13종 -> data/processed/app.db
	cd backend && python scripts/preprocess.py

train:            ## LightGBM 2종 학습 -> backend/models/
	cd backend && python scripts/train_congestion.py && python scripts/train_route_difficulty.py
